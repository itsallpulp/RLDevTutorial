...
void RenderComponent::LoadJson(json::object data)
{
	...
	color = data.contains("color") ? json::value_to<std::string>(data["color"]).at(0) : glyph;
	bgColor = data.contains("bgColor") ? json::value_to<std::string>(data["bgColor"]).at(0) : glyph;
}
